...
/* ------------------------------------------------------------------
 * WinXP Luna titlebar helpers
 * ------------------------------------------------------------------ */

/*
 * Is window w a "chrome" window — panel, launcher, etc. — that should
 * not get a titlebar drawn over it and should treat its whole interior
...
wischrome(Window *w)
{
	...
	&& (w->label == nil || strstr(w->label, "vtwin") == nil))
		return 1;
	if(w->label != nil){
		if(strcmp(w->label, "xena-panel") == 0) return 1;
		if(strcmp(w->label, "launcher")   == 0) return 1;
	}
	return 0;
}

/*
 * wcontentrect: return the inner rectangle that the terminal text lives in,
 * sitting below the titlebar. For chrome windows (panel, launcher) the
 * whole interior IS content.
 */
Rectangle
wcontentrect(Window *w)
{
	Rectangle r;
...
/*
 * whittest: which hit-zone does point pt fall into for window w?
 * Returns one of: 0=none 1=close 2=min 3=max 4=title 5=content 6=border
 */
int
whittest(Window *w, Point pt)
{
	Rectangle tr, cr, btnr;
	int bx;

	if(w->i == nil)
		return 0;
	if(!ptinrect(pt, w->i->r))
		return 0;
	cr = wcontentrect(w);
	if(ptinrect(pt, cr))
		return 5;	/* content */
	...
	/* titlebar strip */
	tr = insetrect(w->i->r, Selborder);
	tr.max.y = tr.min.y + Titlebar;
	if(!ptinrect(pt, tr))
		return 6;	/* border */

	/* buttons are right-aligned: max | min | close */
	bx = tr.max.x - Btnpad - Btnw;
	/* close button */
	btnr = Rect(bx, tr.min.y+(Titlebar-Btnh)/2,
	            bx+Btnw, tr.min.y+(Titlebar-Btnh)/2+Btnh);
	if(ptinrect(pt, btnr))
		return 1;
	bx -= Btnpad + Btnw;
	/* max button */
	...
	return 4;	/* title drag area */
}

/*
 * wdrawtitlebar: paint the WinXP Luna titlebar for window w.
 * active=1  ->  deep blue gradient (focused)
 * active=0  ->  pale blue gradient (unfocused)
 *
 * Gradient is approximated by drawing Titlebar horizontal lines,
 * interpolating from left colour to right colour.  Plan 9 libdraw
 * has no gradient primitive, so we do it the hard way.
 */
void
wdrawtitlebar(Window *w, int active)
{
	Rectangle tr, btnr;
	Image *col, *bcol;
	Point tp;
	int i, bx, by;
	ulong c;
	ulong lr, lg, lb;	/* left RGB */
	ulong rr, rg, rb;	/* right RGB */
	ulong R, G, B;
	Rectangle lr2;

	if(w->i == nil)
		return;
	/* chrome windows (panel, launcher) — no titlebar paint */
	if(wischrome(w))
		return;
	...
	if(active){
		/* #0A246A -> #A6CAF0 */
		lr = 0x0A; lg = 0x24; lb = 0x6A;
		rr = 0xA6; rg = 0xCA; rb = 0xF0;
	} else {
		/* #7A96DF -> #C8D4F5 */
		lr = 0x7A; lg = 0x96; lb = 0xDF;
		rr = 0xC8; rg = 0xD4; rb = 0xF5;
	}

	/*
	 * Gradient: draw Titlebar horizontal lines by interpolating colour.
	 * We allocate ONE reusable 1x1 image and reuse it row by row to avoid
	 * hammering the image pool (22 allocs per repaint was too many).
	 * First row: solid fill.  Then replicate by copying and changing the
	 * fill value.  Simplest: alloc once per unique colour step.
	 * Plan 9 doesn't have fillrect with arbitrary colour; fastest portable
	 * approach is still alloc+draw+free, but we keep it to one per row.
	 * For a 22-row titlebar this is 22 allocs total — manageable if we
	 * free immediately.
	 */
	for(i = 0; i < Titlebar; i++){
		if(Titlebar > 1){
			R = lr + (rr-lr)*i/(Titlebar-1);
			G = lg + (rg-lg)*i/(Titlebar-1);
			B = lb + (rb-lb)*i/(Titlebar-1);
		} else {
			R = lr; G = lg; B = lb;
		}
		c = (R<<24)|(G<<16)|(B<<8)|0xFF;
		col = allocimage(display, Rect(0,0,1,1), screen->chan, 1, c);
		if(col == nil)
			continue;
		lr2.min.x = tr.min.x;
		lr2.min.y = tr.min.y + i;
		lr2.max.x = tr.max.x;
		lr2.max.y = tr.min.y + i + 1;
		draw(w->i, lr2, col, nil, ZP);
		freeimage(col);
	}

	/*
	 * Flush after painting the gradient. The 22 row-paints can otherwise
	 * languish in the libdraw buffer if no flush happens before the
	 * caller returns, which is exposed by the "squished 2-pixel
	 * titlebar" bug for libdraw client windows (the kernel only commits
	 * ~2 rows before the next event preempts the flush).
	 */
	flushimage(display, 1);

	/* title text — window label or fallback */
	tp.x = tr.min.x + Titlepad;
	tp.y = tr.min.y + (Titlebar - font->height)/2;
	if(tp.y < tr.min.y) tp.y = tr.min.y;
	{
		char *lbl = (w->label != nil && w->label[0] != '\0') ? w->label : w->dir;
		if(lbl == nil) lbl = "mxio";
		string(w->i, tp, display->white, ZP, font, lbl);
	}

	/* buttons: right-to-left — close | max | min */
	by = tr.min.y + (Titlebar - Btnh)/2;
	bx = tr.max.x - Btnpad - Btnw;

	/* close: #C75050 */
	bcol = allocimage(display, Rect(0,0,1,1), screen->chan, 1, 0xC75050FF);
	if(bcol){
		btnr = Rect(bx, by, bx+Btnw, by+Btnh);
		draw(w->i, btnr, bcol, nil, ZP);
		line(w->i, Pt(bx+4,by+4), Pt(bx+Btnw-5,by+Btnh-5), 0, 0, 1, display->white, ZP);
		line(w->i, Pt(bx+Btnw-5,by+4), Pt(bx+4,by+Btnh-5), 0, 0, 1, display->white, ZP);
		freeimage(bcol);
	}

	bx -= Btnpad + Btnw;
	/* max: #4A7EBB */
	bcol = allocimage(display, Rect(0,0,1,1), screen->chan, 1, 0x4A7EBBFF);
	if(bcol){
		btnr = Rect(bx, by, bx+Btnw, by+Btnh);
		draw(w->i, btnr, bcol, nil, ZP);
		border(w->i, insetrect(btnr, 3), 1, display->white, ZP);
		freeimage(bcol);
	}
	...
	if(bcol){
		...
		draw(w->i, btnr, bcol, nil, ZP);
		draw(w->i,
		     Rect(bx+3, by+Btnh-5, bx+Btnw-3, by+Btnh-3),
...
wborder(Window *w, int type)
{
	...
	active = (type == Selborder);
	wdrawtitlebar(w, active);
}
...
wresize(Window *w, Image *i)
{
	...
	if(!w->mouseopen || !w->winnameread){
		...
		/* mouseopen clients (vtwin etc.) repaint their own content
		 * area on Aresize, but the new backing image from
		 * allocwindow(Refbackup, DNofill) is garbage. Clear it so
		 * we don't flash old-memory contents while the client gets
		 * around to repainting. The titlebar area gets overpainted
		 * by wborder/wdrawtitlebar below. */
		draw(w->i, w->i->r, cols[BACK], nil, ZP);
	}
	if(w == input)
		wborder(w, Selborder);
	else
...
wmk(Image *i, Mousectl *mc, Channel *ck, Channel *cctl, int scrolling)
{
	...
	if(Dy(i->r) > Titlebar + 20)
		r.min.y += Titlebar;     /* reserve titlebar strip at top */
	w->i = i;
	w->mc = *mc;
	w->ck = ck;
	w->cctl = cctl;
...
wsetpid(Window *w, int pid, int dolabel)
{
	...
	if(pid <= 0)
		...
		if(dolabel){
			...
			free(w->label);
			w->label = estrdup(buf);
			/* redraw titlebar with new pid label */
