...
/*
 * cycle focus to next (dir=+1) or previous (dir=-1) visible window.
 * Wraps. Skips windows with zero-width screenr (hidden).
 */
static void
alttabcycle(int dir)
{
	...
	/* find current input window's index */
	idx = -1;
	for(i = 0; i < nwindow; i++)
		if(window[i] == input){
			idx = i;
	...
	/* walk forward/backward, skipping hidden/deleted */
	n = nwindow;
	for(j = 1; j <= n; j++){
		i = (idx + dir*j + n*n) % n;
		w = window[i];
...
keyboardtap(void*)
{
	...
	for(;;){
		...
		case Akbd:
			...
			/* Alt+Tab / Alt+Shift+Tab — cycle focus, swallow event. */
			if(*s == 'c' && altdown && s[1] == '\t' && s[2] == '\0'){
				alttabcycle(shiftdown ? -1 : +1);
				free(s);
				break;
			}
			if(totap == nil)
	...
	for(;;)
		...
		Again:
			...
			if(winput!=nil && !winput->deleted && winput->i!=nil){
				/* WinXP-style: button-1 click on a DIFFERENT window's
				 * titlebar buttons fires the action immediately instead
				 * of just focusing the window and draining the click.
				 * Lets users close/min/max any window with one click. */
				if(mouse->buttons == 1){
					Window *wu = wpointto(mouse->xy);
					if(wu != nil && wu != winput && wu->i != nil
					   && Dy(wu->i->r) > Titlebar + 20){
						int hz2 = whittest(wu, mouse->xy);
						if(hz2 == 1){
							while(mouse->buttons)
								readmouse(mousectl);
							if(whittest(wu, mouse->xy) == 1){
								incref(wu);
								wsendctlmesg(wu, Deleted, ZR, nil);
								wclose(wu);
							}
							goto Again;
						}
						...
						if(hz2 == 3){
							...
							while(mouse->buttons)
								readmouse(mousectl);
							if(whittest(wu, mouse->xy) == 3){
								if(wu->maximized){
									target = wu->savedrect;
									wu->maximized = 0;
								} else {
									wu->savedrect = wu->screenr;
									target = screen->r;
									target.max.y -= 30;
									wu->maximized = 1;
								}
								ni = allocwindow(wscreen, target, Refbackup, DNofill);
								if(ni != nil){
									incref(wu);
									wsendctlmesg(wu, Reshaped, target, ni);
									wclose(wu);
								}
							}
							goto Again;
				...
				/* --- WinXP titlebar hit testing --- */
				/* The titlebar (close/min/max/drag) belongs to mxio
				 * and must work even if the client has /dev/mouse
				 * open. We only fall through to the client (Sending)
				 * for clicks below the titlebar. Skip entirely for
				 * short windows (panel, launcher) where the titlebar
				 * would occupy most/all of the height — those windows
				 * handle their own mouse events. */
				if(mouse->buttons == 1
				   && Dy(winput->i->r) > Titlebar + 20){
					int hz = whittest(winput, mouse->xy);
					if(hz == 1){
						/* Close button — drain button, then delete window */
						while(mouse->buttons)
							readmouse(mousectl);
						/* only close if still over close button */
						if(whittest(winput, mouse->xy) == 1){
							incref(winput);
							wsendctlmesg(winput, Deleted, ZR, nil);
							wclose(winput);
						}
						goto Again;
					}
					...
					if(hz == 3){
						...
						while(mouse->buttons)
							readmouse(mousectl);
						/* re-check still over max button */
						if(whittest(winput, mouse->xy) == 3){
							if(winput->maximized){
								/* restore to remembered geometry */
								target = winput->savedrect;
								winput->maximized = 0;
							} else {
								/* save current, go fullscreen.
								 * Reserve 30px at bottom for xena-panel. */
								winput->savedrect = winput->screenr;
								target = screen->r;
								target.max.y -= 30;
								winput->maximized = 1;
							}
							/* allocwindow new backing image — same pattern as wctl.c */
							ni = allocwindow(wscreen, target, Refbackup, DNofill);
							if(ni != nil){
								incref(winput);
								wsendctlmesg(winput, Reshaped, target, ni);
								wclose(winput);
							}
						}
						goto Again;
				...
				/* --- end titlebar hit testing --- */

				inside = ptinrect(mouse->xy, wcontentrect(winput));
				if(winput->mouseopen)
					scrolling = FALSE;
				else if(scrolling)
			...
			if(mouse->buttons){
				...
				if(w==nil || (w==winput && w->topped>0)){
					...
					if(wtop(mouse->xy) && (mouse->buttons!=1 || inborder(w->screenr, mouse->xy)))
						goto Again;
					/* WinXP-style: for short windows (panel, launcher), top
					 * AND pass the click through so the panel's own mouse
					 * handler can react to button-1 clicks immediately. */
					if(w != nil && w->i != nil && Dy(w->i->r) <= Titlebar + 20){
						wtop(mouse->xy);
						goto Again;
					}
					goto Drain;
...
button3menu(void)
{
	...
	case MxNew:
	{
		/* spawn terminal at a sensible fixed size */
		Image *ni;
		r = screen->r;
		r.min.x += 50;
		r.min.y += 130;
...
sweep(void)
{
	...
	while(mouse->buttons == 4){
		if(!eqpt(mouse->xy, p)){
			...
			if(Dx(r)>5 && Dy(r)>5){
				/* Refbackup (not Refnone): when this transient
				 * rubber-band image is later freed (next iter or
				 * Rescue), devdraw recomposes the background
				 * underneath. With Refnone, orphan pixels are
				 * left behind whenever the next rect doesn't
				 * fully cover this one (drag inward, abort,
				 * client crash mid-paint). See wiki concept
				 * mxio-design.md "GOTCHA: sweep orphan pixels".
				 */
				i = allocwindow(wscreen, r, Refbackup, DNofill);
				freeimage(oi);
				if(i == nil)
