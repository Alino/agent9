...
func (p *Program) handleResize() chan struct{} {
	...
	} else if vtsListenForResize(p, ch) {
		// plan9/vts: our shim handles resize via the vts ctl file,
		// not via the standard ttyOutput path. listenForResize on
		// plan9 reads /n/vts/<s>/ctl and sends WindowSizeMsg.
	} else {
		close(ch)
	}
...
// vtsListenForResize starts the plan9-vts resize watcher when running
// inside a vts session. On other platforms (or when not in vts) it
// returns false and the caller closes the channel itself. The plan9
// build provides the real implementation in tty_plan9.go; everywhere
