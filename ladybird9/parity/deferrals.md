...
|---|---|---|---|
| `<video>`/`<audio>` decode (RESOLVED) | ENABLED. ffmpeg 7.1.1 (avcodec/avformat/avutil/swresample) cross-built C-only (host/deps/build-ffmpeg.sh; --disable-asm/x86asm/inline-asm — NASM can't go through cc9→a.out, C fallbacks decode correctly, slower; decoders+demuxers+parsers only, no encoders/muxers/programs). LibMedia links the real FFmpeg TUs + real Codecs/Vorbis.cpp (stubs retired); ENABLE_VIDEO/AUDIO ON (plan9-inject). Two cc9 C11-conformance fixes it forced: `<assert.h>` now defines the `static_assert` macro, `<errno.h>` gained EFBIG/ETXTBSY. All 6 FFmpeg glue TUs + Vorbis compile; WebContent links the quartet. Audio *output* is now wired: `Audio/PlaybackStreamPlan9.cpp` (selected by Meta/CMake/audio.cmake's PLAN9 arm) opens `/dev/audio`, pulls float samples from the data-request callback on a control thread, converts to interleaved s16 with volume, and blocking-writes them (44100 Hz stereo). A missing `/dev/audio` degrades to a paced silent sink (decode still works). **Runtime decode + output VALIDATED on cirno**: an autoplay <video> (H.264/mp4) shows its decoded first frame in a headless screenshot (cyan ~64x64); and in an interactive rio window a 3s A+V clip (AAC) drives PlaybackStreamPlan9::create() and writes 536,576 bytes to /dev/audio (= 3.04 s at 44100×2×2) — audio flows end-to-end. NOTE: audio-out only instantiates in interactive mode; upstream `HTMLMediaElement` disables the audio sink when `is_headless()` (so headless screenshots decode video but never open the device). | asm-accelerated decode (needs a Plan 9 a.out assembler path) |
...
| Response-body delivery (RESOLVED 2026-07-24) | Not a deferral — a fixed port bug, recorded because the mechanism is Plan 9-specific and the workarounds are visible. RequestServer downloaded every resource fine but WebContent never drained the response pipe, so pages rendered blank (the main document itself arrived only partly, so the parser never finished). Two causes: (a) the pipe fd arrives BLOCKING — cc9 has no SCM_RIGHTS, so LibIPC passes fds by name through `/srv` and the receiver `open`s a fresh, blocking fd; the drain loop then blocks inside the notifier callback and wedges the event loop; (b) the readable/writable `Core::Notifier`s on that pipe stop firing mid-body (708 EAGAINs vs 6 activations observed). Fixes, all `#if defined(AK_OS_PLAN9)`: force the received socket/FIFO fd non-blocking (regular files must NOT be forced — they then return EAGAIN and the loader reports a failed `file://` load); close the read notifier only once `request_done` is set (a merely-drained non-blocking pipe reports `is_eof()`, and closing there stops delivery permanently); re-drive both the read drain and the queued writes on 10 ms timers; a 20 ms connection-level timer that pumps `curl_multi_socket_action`; and `CURLOPT_LOW_SPEED_LIMIT/TIME` so a transfer that connects and goes silent aborts instead of wedging the parser behind a render-blocking stylesheet. | patches 0005 (TransportPlan9), 0011 (progress timer), 0013 (delivery) | — |
| Poll ring size | Multi-MB resources need `CC9_POLL_RING` raised (the launcher/run scripts set 8 MiB). cc9's poll layer buffers a whole transfer in a per-fd ring; at the 64 KiB default plus Plan 9's fixed 256 KiB pipe buffer, a 10 MB script (youtube's `base.js` bundle) stalls at ~320 KB. Env-tunable as of cc9 commit 313cf89. | `cc9/runtime/poll.c` reads `CC9_POLL_RING` (clamped 4 KiB–64 MiB) | a growable ring, or flow control that doesn't need one |
| `navigator.getBattery()` | HIDDEN (commented out in `Navigator.idl`). LibWeb has no `BatteryManager`, so upstream's binding exists but ALWAYS rejects with NotAllowedError; youtube.com calls it without a `.catch()` and the unhandled rejection kills the player init chain. Firefox ships no Battery API at all and sites cope, so absence is the safer shape. **Symmetric** — the IDL has no preprocessor, so the host parity build loses it too and the byte-identical comparison still holds. | patch 0014 | `BatteryManager` is implemented |
| Autoplay at HAVE_FUTURE_DATA | Upstream runs the autoplay substeps only in the HAVE_ENOUGH_DATA branch of `set_ready_state`. An MSE source that buffers just ahead of the playhead never reaches "5 s ahead" or EOF while paused, so it stays at HAVE_FUTURE_DATA forever and autoplay never fires — the video sits on its first frame. The same substeps now also run in the HAVE_FUTURE_DATA branch (guarded by `is_eligible_for_autoplay()`, which requires `paused()`, so it cannot re-fire). **Symmetric**, as above. | patch 0014 | upstream adopts an equivalent rule, or the buffering heuristic reaches HAVE_ENOUGH_DATA on its own |
| youtube.com consent wall | `LB9_YT_CONSENT=1` seeds the SOCS/CONSENT cookies a user's "accept" click would set. Headless starts with an empty cookie jar, so youtube.com serves its "Before you continue" interstitial instead of the watch page. Opt-in, off by default, PLAN9-guarded. | patch 0014 (`LibWebView/Application.cpp`) | a real UI consent click, or a persistent profile carrying the cookie |
