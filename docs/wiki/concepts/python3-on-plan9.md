...
- **99.72%** (6104/6121) over *applicable* testcases (ref-passing, not on the
  justified skip-list) across the **39 modules that run to completion**.
- 136 justified skips, all with one-line reasons in `skiplist.txt`: 32-bit
  ...
  tests, locale. 17 regressions remain *visible* (not hidden): **47 are the
