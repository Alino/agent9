...
// Plan 9 (9front) over cc9's poll(2) + a self-pipe waker. No epoll/kqueue here,
// and the wasip1 poll-based backend is explicitly single-threaded-only, so this
// is its own module. See sys/plan9/mod.rs.
#[cfg(target_os = "plan9")]
cfg_os_poll! {
    mod plan9;
    pub(crate) use self::plan9::*;
}
