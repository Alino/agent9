...
impl TcpSocket {
    ...
    pub async fn connect(self, addr: SocketAddr) -> io::Result<TcpStream> {
        if let Err(err) = self.inner.connect(&addr.into()) {
            #[cfg(not(windows))]
            // NB plan9: cc9's connect() is synchronous even on a non-blocking
            // socket (a /net connect is a write to the conn's ctl file), so it
            // never reports EINPROGRESS — it has already completed or failed by
            // the time we get here. The comparison is harmless; the cost is that
            // the handshake blocked this thread. See servo9 notes.
...
#[cfg(any(unix, target_os = "plan9", target_os = "wasi"))]
cfg_unix_or_wasi! {
