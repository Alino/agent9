...
// Plan 9 (9front). Not `unix`, and its conventions differ in two ways that
// matter here: the home directory is named by **`$home`** (lowercase — `$HOME`
// does not exist), and there is no XDG spec, so no user_dirs to enumerate.
#[cfg(target_os = "plan9")]
mod target_plan9 {
...
pub fn home_dir() -> Option<PathBuf> {
    // Plan 9 publishes the environment in /env; rc sets $home, not $HOME.
    env::var_os("home")
        .filter(|s| !s.is_empty())
        .map(PathBuf::from)
}
...
pub fn user_dir(_user_dir_name: &str) -> Option<PathBuf> {
    // No XDG user-dirs spec on Plan 9. Returning None is honest: there is no
    // configured Desktop/Downloads/... to point at, and inventing paths under
    // $home would create directories the user never asked for.
    None
}
...
#[cfg(target_os = "plan9")]
pub use self::target_plan9::{home_dir, user_dir, user_dirs};
