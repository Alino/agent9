...
impl Verifier {
    ...
    fn new_inner(
        ...
        // Plan 9 (9front) has no system trust store: nothing to read from disk and
        // no platform verifier to ask. The unix arm above is `cfg(unix)` and Plan 9
        // is not unix, so without this arm no roots load at all, the store stays
        // empty, and Verifier::new fails with NoRootAnchors — which Servo turns
        // into a startup panic, since it prewarms this verifier even for plain
        // http:// pages.
        //
        // So take the same compiled-in Mozilla root program the wasm32 arm uses,
        // for the same reason.
        //
        // ponytail: these roots are baked into the binary, so the trusted set can
        // only be changed by rebuilding — a revoked CA stays trusted until then,
        // and a private/corporate CA cannot be added at all. The upgrade path is an
        // $SSL_CERT_FILE override (what rustls-native-certs does on unix); it needs
        // a PEM parser, and rustls-pemfile is not in this tree today.
        #[cfg(target_os = "plan9")]
        {
            root_store.add_parsable_certificates(
                webpki_root_certs::TLS_SERVER_ROOT_CERTS.iter().cloned(),
