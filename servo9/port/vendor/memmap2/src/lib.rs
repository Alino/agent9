...
// Plan 9 (servo9 port): a real backend instead of the all-Unsupported stub —
// read-into-heap mappings, since the platform has no file-backed mmap at all.
// Without this, every Mmap::map() fails silently at runtime (this is how Servo's
// fonts refused to load with the TTF sitting right there). See plan9.rs.
#[cfg_attr(target_os = "plan9", path = "plan9.rs")]
#[cfg_attr(not(any(unix, windows, target_os = "plan9")), path = "stub.rs")]
mod os;
