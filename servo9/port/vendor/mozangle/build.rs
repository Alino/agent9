...
fn build_windows_dll(data: &build_data::Data, dll_name: &str, def_file: &str) {
    ...
    if env::var("CARGO_CFG_TARGET_OS").as_deref() == Ok("plan9") {
        build.define("EGL_NO_PLATFORM_SPECIFIC_TYPES", None);
    }
...
fn build_single_lib(compiled_libraries: &mut HashSet<Libs>, target: &String, lib: Libs) {
    ...
    // Plan 9 has no window system for eglplatform.h to derive EGLNativeWindowType
    // et al from. ANGLE's own escape hatch makes them void*, which is correct
    // here: Servo uses this crate as a shader translator only.
    if env::var("CARGO_CFG_TARGET_OS").as_deref() == Ok("plan9") {
        build.define("EGL_NO_PLATFORM_SPECIFIC_TYPES", None);
    }
    ...
    if matches!(lib, Libs::ANGLE_COMMON) {
        ...
        for &(os, sources) in &[
            ...
            (
                // Plan 9 (cc9): POSIX-shaped, and ANGLE is used here only as a
                // shader translator — no native GL/EGL backend to support.
                "plan9",
                &[
                    "gfx/angle/checkout/src/common/system_utils_posix.cpp",
                ][..],
...
fn build_translator(compiled_libraries: &mut HashSet<Libs>, target: &String) {
    ...
    // See build_single_lib: Plan 9 has no window system for eglplatform.h to
    // derive EGLNativeWindowType et al from, and ANGLE is used here purely as a
    // shader translator.
    if env::var("CARGO_CFG_TARGET_OS").as_deref() == Ok("plan9") {
        clang_args.push("-DEGL_NO_PLATFORM_SPECIFIC_TYPES".to_string());
    }
