...
#    define ANGLE_PLATFORM_POSIX 1
    ...
    defined(__QNX__) || defined(__Fuchsia__) || defined(__HAIKU__) || defined(__plan9__)
// Plan 9 (cc9): not a "platform" ANGLE knows, but everything this switch gates
// is POSIX-shaped — pthreads, pthread_key_t TLS, errno — and cc9 provides all of
// it. ANGLE is only used here as a shader translator (no native GL backend), so
// the generic POSIX arm is exactly right.
#    define ANGLE_PLATFORM_POSIX 1
#else
