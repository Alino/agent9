...
#[cfg(all(
    any(not(target_os = "android"), feature = "dynamic_freetype"),
    not(target_os = "plan9")
))]
...
// On Android (and Plan 9, which has no dynamic linker at all), just
// statically link in the symbols...
#[cfg(any(
    all(target_os = "android", not(feature = "dynamic_freetype")),
    target_os = "plan9"
))]
macro_rules! ft_dyn_fn {
