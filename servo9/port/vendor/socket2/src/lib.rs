    ...
    ($from: ty, $for: ty) => {
        impl From<$from> for $for {
            fn from(socket: $from) -> $for {
                // plan9 (cc9) is fd-based like unix — std::os::fd is implemented
                // for it (a /net connection is one fd; FromRawFd rebuilds the
                // conn dir via fd2path). Without plan9 here the body compiles to
                // `()` and every `from!` fails with "expected Socket, found ()".
                #[cfg(any(
                    unix,
                    target_os = "plan9",
...
// Plan 9 (cc9) uses the unix backend: cc9's sockets ARE BSD sockets over /net
// (runtime/net9.c), so socket()/bind()/setsockopt()/sendmsg() are the real thing.
// Plan 9 is simply not `unix`, which is the only reason it was excluded.
#[cfg_attr(
    any(
        unix,
        target_os = "plan9",
