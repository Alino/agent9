...
// plan9 (like wasi) has no unix-domain sockets, so nothing here needs to turn a
// Path into sun_path bytes.
#[cfg(not(any(target_os = "wasi", target_os = "plan9")))]
use std::os::unix::ffi::OsStrExt;
#[cfg(all(feature = "all", unix, not(target_os = "plan9")))]
...
#[cfg(any(
    ...
    target_os = "vita",
    target_os = "cygwin",
    // cc9's msghdr has `int msg_iovlen` (see cc9/runtime/include/sys/socket.h).
    target_os = "plan9",
))]
type IovLen = c_int;
...
// Builds an AF_UNIX address. Plan 9 has no unix-domain sockets — same as wasi.
#[cfg(not(any(target_os = "wasi", target_os = "plan9")))]
...
impl SockAddr {
    ...
    // plan9 has no unix-domain sockets and no std::os::unix.
    ...
    // plan9 has no unix-domain sockets, so there is no sun_path to interpret
    // (and no std::os::unix::ffi::OsStrExt to interpret it with).
    #[cfg(not(target_os = "plan9"))]
