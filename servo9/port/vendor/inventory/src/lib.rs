            ...
            #[cfg_attr(
                all(
                    ...
                    any(
                        ...
                        target_os = "vxworks",
                        target_os = "none",
                        // Plan 9 (9front, via cc9): its crt0 walks .init_array
                        // before main exactly like the ELF platforms above. Being
                        // absent from this list is not a no-op — the static then
                        // gets no section at all, so the constructor is never run
                        // and every submit! silently vanishes, leaving inventory
                        // iteration empty at runtime with nothing to diagnose.
                        target_os = "plan9",
                    )
                ),
