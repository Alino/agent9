...
/* The entry module's completion promise, settled by the main event loop (see below).
 * JS_UNDEFINED until an ES-module entry point is evaluated. */
static JSValue n9_entry_promise;
static int n9_entry_promise_set;
...
static int eval_buf(JSContext *ctx, const char *buf, size_t len, const char *fn, int is_main) {
    ...
    if (JS_DetectModule(buf, len)) {
        ...
        if (is_main && !JS_IsException(val)) {
            /* Do NOT js_std_await() the entry module. That helper runs exactly ONE pending
             * job per iteration and then polls, and the poll blocks until the next timer
             * expires — so an entry module with a top-level `await` advances one microtask
             * per timer deadline. With a 5-minute idle timer armed (pi arms one) the
             * program looks hung. js_std_loop() in main() drains the whole job queue
             * before each poll; hand the promise to it and check the result afterwards. */
            n9_entry_promise = val;
            n9_entry_promise_set = 1;
            return 0;
        }
...
int main(int argc, char **argv) {
    ...
    js_std_loop(ctx);
    /* The entry module ran to completion inside the loop above; report a rejection the way
     * js_std_await() would have, now that nothing is left to wait for. */
    if (n9_entry_promise_set) {
        if (JS_PromiseState(ctx, n9_entry_promise) == JS_PROMISE_REJECTED) {
            JS_Throw(ctx, JS_PromiseResult(ctx, n9_entry_promise));
            js_std_dump_error(ctx);
            ret = -1;
        }
        JS_FreeValue(ctx, n9_entry_promise);
        n9_entry_promise_set = 0;
    }
