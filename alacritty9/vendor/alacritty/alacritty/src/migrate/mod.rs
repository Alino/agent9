    ...
    P: AsRef<Path> + Debug,
{
    ...
    if options.dry_run && !options.silent {
        ...
        // No tempfile crate on Plan 9; a same-directory rename is atomic.
        #[cfg(target_os = "plan9")]
        {
            let tmp = path.with_extension("migrate-tmp");
            fs::write(&tmp, toml).map_err(|err| format!("filesystem error: {err}"))?;
            fs::rename(&tmp, path).map_err(|err| format!("atomic replacement failed: {err}"))?;
        }
    }
    Ok(())
}
