...
// On Plan 9 stdout is the GL9F frame stream to gl9win2 — a single log line
// there kills the window ("bad frame magic"). All console logging goes to
// stderr instead (fd 2 passes through gl9win2).
#[cfg(not(target_os = "plan9"))]
type ConsoleStream = Stdout;
#[cfg(target_os = "plan9")]
type ConsoleStream = io::Stderr;

#[cfg(not(target_os = "plan9"))]
fn console_stream() -> ConsoleStream {
    io::stdout()
}
#[cfg(target_os = "plan9")]
fn console_stream() -> ConsoleStream {
...
pub struct Logger {
    ...
    stdout: Mutex<LineWriter<ConsoleStream>>,
...
impl Logger {
    fn new(event_proxy: EventLoopProxy<Event>) -> Self {
        ...
        let stdout = Mutex::new(LineWriter::new(console_stream()));
    ...
    fn file(&mut self) -> Result<&mut LineWriter<File>, io::Error> {
        ...
        if self.file.is_none() {
            ...
            match file {
                ...
                Err(e) => {
                    let _ = writeln!(console_stream(), "Unable to create log file: {e}");
                    return Err(e);
                },
            }
        }
