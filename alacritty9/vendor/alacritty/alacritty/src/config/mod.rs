...
pub mod monitor;
// The notify crate (inotify/kqueue) has no plan9 build; live config reload is
// stubbed out.
#[cfg(target_os = "plan9")]
#[path = "monitor_plan9.rs"]
pub mod monitor;
pub mod scrolling;
pub mod selection;
pub mod serde_utils;
...
/// Plan 9 config locations: $home/lib/alacritty/, then $home/.config/alacritty/
/// (for people carrying a unix config over), then /lib/alacritty/.
#[cfg(target_os = "plan9")]
pub fn installed_config(suffix: &str) -> Option<PathBuf> {
    let file_name = format!("alacritty.{suffix}");

    if let Some(home) = home::home_dir() {
        let candidate = home.join("lib/alacritty").join(&file_name);
        if candidate.exists() {
            return Some(candidate);
        }
        let candidate = home.join(".config/alacritty").join(&file_name);
        if candidate.exists() {
            return Some(candidate);
        }
    }

    let fallback = PathBuf::from("/lib/alacritty").join(&file_name);
