...
/// plan9 (cc9) has no `libc` crate. This file only needs the stdio FILENO
/// constants from it, and those are 0/1/2 on Plan 9 as everywhere else, so
/// stand in for `libc` the same way hermit aliases `hermit_abi`.
#[cfg(target_os = "plan9")]
mod libc {
...
#[cfg(not(target_os = "trusty"))]
#[cfg(not(target_os = "plan9"))] // plan9: Pipe is not FileDesc-based yet
