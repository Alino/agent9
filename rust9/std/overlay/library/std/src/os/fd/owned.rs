...
impl BorrowedFd<'_> {
    ...
    /// Creates a new `OwnedFd` instance that shares the same underlying file
    /// description as the existing `BorrowedFd` instance.
    /// plan9 (cc9): same F_DUPFD_CLOEXEC strategy as the default arm — cc9's
    /// fcntl implements it (runtime/poll.c) — reached through cc9 directly since
    /// the `libc` crate has no plan9 target.
    #[cfg(target_os = "plan9")]
    #[stable(feature = "io_safety", since = "1.63.0")]
    pub fn try_clone_to_owned(&self) -> io::Result<OwnedFd> {
        unsafe extern "C" {
        ...
        const F_DUPFD_CLOEXEC: i32 = 1030; // cc9 <fcntl.h>
        let fd = crate::sys::cvt(unsafe { fcntl(self.as_raw_fd(), F_DUPFD_CLOEXEC, 3) })?;
        Ok(unsafe { OwnedFd::from_raw_fd(fd) })
    }
...
impl Drop for OwnedFd {
    ...
    fn drop(&mut self) {
        unsafe {
            ...
            #[cfg(not(any(target_os = "hermit", target_os = "plan9")))]
            {
                #[cfg(unix)]
            ...
            // plan9 (cc9): the `libc` crate has no plan9 target, and the rest of
            // this std references it nowhere — close through cc9 directly.
            #[cfg(target_os = "plan9")]
            {
                unsafe extern "C" {
                    fn close(fd: i32) -> i32;
                }
                let _ = close(self.fd.as_inner());
            }
...
#[stable(feature = "io_safety", since = "1.63.0")]
// plan9: needs into_socket()/from_inner on the /net-native types — the
// conn dir can be rebuilt from a bare fd via fd2path(2), so this is
// doable, just not the Berkeley one-liner. Deferred.
#[cfg(not(any(target_os = "trusty", target_os = "plan9")))]
...
// plan9: implemented in terms of the FromRawFd/IntoRawFd impls in os/fd/net.rs,
// which are real (fd2path recovers the /net conn directory from a bare fd).
#[cfg(target_os = "plan9")]
macro_rules! impl_owned_fd_conv_plan9 {
    ($($t:ident)*) => {$(
        #[stable(feature = "io_safety", since = "1.63.0")]
        impl From<crate::net::$t> for OwnedFd {
            #[inline]
            fn from(t: crate::net::$t) -> OwnedFd {
                // SAFETY: into_raw_fd transfers ownership of a valid fd.
                unsafe { OwnedFd::from_raw_fd(t.into_raw_fd()) }
            }
        }

        #[stable(feature = "io_safety", since = "1.63.0")]
        impl From<OwnedFd> for crate::net::$t {
            #[inline]
            fn from(owned_fd: OwnedFd) -> Self {
                // SAFETY: OwnedFd guarantees the fd is open and owned here.
...
#[cfg(target_os = "plan9")]
impl_owned_fd_conv_plan9! { TcpStream TcpListener UdpSocket }
