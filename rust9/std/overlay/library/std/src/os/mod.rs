...
#[cfg(any(
    ...
    target_os = "wasi",
    target_os = "motor",
    // plan9 (cc9): fds are real and a TcpStream is one of them, so these types
    // are meaningful. socket2/mio/tokio are written entirely in terms of them.
    target_os = "plan9",
    doc
))]
pub mod fd;
...
pub mod hermit;
// Plan 9 paths are byte strings like unix's, so it needs the same OsStrExt —
// under its own name, since it is not `unix`. (Same pattern as hermit.)
#[cfg(target_os = "plan9")]
pub mod plan9;
