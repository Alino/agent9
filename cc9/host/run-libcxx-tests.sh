...
for t in $(echo "$sample" | head -"$N"); do
  ...
  if ! "$LLVM/clang++" --target=x86_64-unknown-none -nostdlib -DNDEBUG -std=c++23 -nostdinc++ \
        ...
        -fno-exceptions -fno-rtti -fno-threadsafe-statics -D_LIBCPP_PROVIDES_DEFAULT_RUNE_TABLE -D_LIBCPP_HAS_CLOCK_GETTIME -femulated-tls $addf -c "$t" -o /tmp/lt.o 2>/dev/null; then
