...
"$LLVM/clang++" "${base[@]}" -std=c++20 -isystem "$LIBCXX" -isystem "$INC" -c "$CC9/runtime/cxxrt.cpp" -o "$O/cxxrt.o"
...
for f in string stdexcept memory hash functional memory_resource system_error error_category; do
