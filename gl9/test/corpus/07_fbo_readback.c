/* 07_fbo_readback — render-to-texture through an FBO, then glReadPixels it back.
 *
 ...
 *   GALLIUM_DRIVER=softpipe GALLIUM_NOSSE=1 07_fbo_readback
 * Expects "07_fbo_readback N/N PASS".
 */
#include <stdio.h>
#include <string.h>
#include <EGL/egl.h>
...
main(void)
{
	...
	snprintf(d, sizeof d, "got RGBA(%d,%d,%d,%d) want ~(64,128,191,255)",
	         px[0], px[1], px[2], px[3]);
	ok("FBO readback has the cleared colour",
